...
class ED60S(object):
	...
	def GetWebPage(self):
		...
		try:
			if self.webDate.text[5:15] == ("%d-%02d-%02d"%(self.date[0],self.date[1],self.date[2])):
				...
				print("404 Not Found.")
				self.isError = True
				self.errorText += "404 Not Found.\n"
		except Exception as e:
			print("Not found the web page.")
			self.isError = True
			self.errorText += "Not found the web page.\n"
		if os.path.isfile(f'./Img/Everyday60s-{"%d.%02d.%02d"%(self.date[0],self.date[1],self.date[2])}.png'):
			return f'{self.d60Url}[CQ:image,file=file:///{os.getcwd()}/Img/Everyday60s-{"%d.%02d.%02d"%(self.date[0],self.date[1],self.date[2])}.png]'
		elif self.d60Url is None:
			print("Find Link Error.")
			self.isError = True
			self.errorText += "Find Link Error.\n"
		else:
			self.Day60s()
		if self.isError:
	...
	def Day60s(self):
		...
		try:
			paragraph = day60.html.find('div.css-376mun>div>p')
		except:
