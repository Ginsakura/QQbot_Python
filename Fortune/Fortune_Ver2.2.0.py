##Version 2.2.0##更新数据存储格式##由ini格式转变为SQLite数据库##失败##
...
def New_Table(Pre):##运势数据库新建模块##已完成##未测试##
    global Path,File,Table,date
    if Pre == False :
        os.makedirs(Path)
    New_Database_Time = sqlite3.connect(Path+File_Time)
    cur = New_Database_Time.cursor()
    ...
    cur.execute(f'''
        ...
        Main integer,
        Extra integer)''')
    cur.execute(f'insert into {date} values(?,?,?,?)', (2602961063,time,'0','0'))
    New_Database.commit()
    New_Database.close()

def User_Data_Read(cur):##用户数据读取模块##已完成##未测试##
    global user,date
    cur.execute(f'select * from sqlite_master where type=\'table\' and name = \'{date}\'')
    cur.execute(f'select * from {Table} where User={user}')
    read = cur.fetchone()
    user,date,main,extra = read
...
def main(user,main_main,Extra_main,repeat,ini_file):##一阶(主)判定模块##已完成##初步测试通过##等待完整测试##
