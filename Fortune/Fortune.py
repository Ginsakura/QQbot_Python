...
class Fortune():
    ...
    def Premise(self):#数据库路径状态判断模块
        ...
        if not os.path.isfile(f'{self.Path}{self.File}') :
            ...
            cursor.execute(f'insert into \'Fortune\' values(?,?,?,?,?,?)', (000000,000000,'0000-00-00','00:00:00',-1,-1))
            New_Database.commit()
            New_Database.close()
    ...
    def Table_Exist(self):#检查当天的表是否存在
        exist = self.cur.execute(f'select * from sqlite_master where type=\'table\' and name=\'Fortune\'')
    ...
    def User_Data_Read(self):
        data = self.cur.execute(f'select * from \'Fortune\' where User=\'{self.user}\' and Date=\'{self.date}\'')
        Read_Data = data.fetchone()
        if Read_Data == None :
            self.Random(statu=True)
        else :
            self.user,self.group,self.date,self.time,self.main_num,self.extra_num = Read_Data
    ...
    def User_Data_Write(self):
        self.cur.execute(f'insert into \'Fortune\' values(?,?,?,?,?,?)', (self.user,self.group,self.date,self.time,self.main_num,self.extra_num))
        self.DB.commit()
...
if __name__ == '__main__':
    #s = Fortune(sys.argv[-2],sys.argv[-1])
    s = Fortune(123456,1234)
    s.Main()
