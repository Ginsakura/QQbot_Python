...
def User_Data_Read():
    ...
    if Read_Data == None :
        ...
        Output(Statu=True,result,time)

def User_Data_Write(result,main,extra):
    cur.execute(f'insert into {date} values(?,?,?,?)', (user,time,main,extra))
    DB.commit()
    DB.close()
    Output(Statu=False,result,time=None)

def Output(Statu,result,time):##输出##
    ST=Solar_Terms()
    if Statu = True:
...
def Solar_Terms():
    ...
    req.group()
    '''
    data = cur.execute(f'select * from Solar_Terms where Year={year} and Month={month} and Day={day}')
    Read_Data = data.fetchone()
    if Read_Data == None:
        ST = ''
    else:
        ST = f'今天是{Read_Data[4]}哦……'
    pass
