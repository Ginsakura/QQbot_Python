...
def Database_write(Database,cur,user,favorability,purview,argument,operation,repeat):##数据写入模块##已完成##未测试##
...
def Output(user,purview,argument,operation,repeat):##输出模块##
    if repeat > 0 :
        repeats = '\n（重复触发）'
    else :
        repeats = ''
...
def Logic(Database,cur,user,status,Purview,favorability,operation,repeat):##逻辑判断模块##
    if status == '' :
        pass
    if repeat > 0 :
        if operation == opera :
        ...
        pass
    Database_write(Database,cur,user,favorability,purview,argument,operation,repeat)
