...
def Get_Web_Page(url):
	...
	try:
		...
		webDate = web.html.find("div.List-item>div.List-itemMeta>div.ActivityItem-meta",first=True)
		#print(date_web.text)
	except:
		return "LinkError!"
	url60 = ''
	if webDate.text[5:15] == ("%d-%02d-%02d"%(year,month,day)) :
	...
	print(url60)
	# try:
	# 	webDate = web.html.find("div.List-item>div.List-itemMeta>div.ActivityItem-meta",first=True).text[5:15]
	# 	if webDate == ("%d-%02d-%02d"%(year,month,day)) :
	# 		dayLink = date_web.absolute_links
	# 	else:
	# 		print("404 Not Found.")
	# 	url60 = "".join(dayLink)
...
def DownloadImg(imgUrl):
	...
	try:
		#保存图片
		r_save_pic = html().get(src, headers = UA)
		# r.content
		with open(f"./Img/Everyday60s-{year}.{month}.{day}.png",'wb+') as fp:
