...
def Make_Image(headimage, date, text):#头图bin，日期，正文传入
	...
	fontPath = './LXGW.ttf'
	#创建字体对象
	text_font = ImageFont.truetype(fontPath, 25)
	date_font = ImageFont.truetype(fontPath, 35)
	...
	draw.multiline_text((130, 560), text=date, fill='#ff9922', font=date_font, spacing=5)
	#边框绘制
	draw.rectangle([(18,18), (782,hight-18)], outline='#444444', width=4)
	draw.line([(24,460), (776,460)], fill='#444444', width=4)
	draw.line([(22,550), (778,550)], fill='#666666', width=2)
	draw.line([(22,610), (778,610)], fill='#666666', width=2)
	for i in line:
		draw.line([(22,630+((i)*29)), (778,630+((i)*29))], fill='#888888', width=2)
	#文本写入
...
def Hight(text):
	...
	line = list()
	for i in text:
		lineNum += len(i)//29+1		#25字/行，整除，行间距1行
		if not len(i)%29 == 0:		#判断是否满行
			lineNum += 1
		line.append(lineNum-1.5)
	mainTextHight = lineNum*29		#正文总高度
	del line[len(line)-1]
