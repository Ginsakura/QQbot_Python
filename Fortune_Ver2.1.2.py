...
def Premise(user,time):##判断文件状态、用户数据读取模块##已完成##测试通过##
    ...
    ini.read(Path + File, encoding="utf-8")
    if ini.has_section(user) == True :
        main_Premise = int(ini.get(user,'main'))
        Extra_Premise = int(ini.get(user,'Extra'))
        main(user,time,main_Premise,Extra_Premise,repeat=True,ini_file=Path + File)
    else :
        main(user,time,None,None,repeat=False,ini_file=Path + File)
